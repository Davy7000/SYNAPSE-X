...
async function setAFK(userJid, reason = "j'ai la flemme de répondre") {
  const now = new Date();
  const afkData = {
    reason: reason,
    setAt: now,
    ...
    if (isAFK(userJid)) {
      if (!input) {
        ...
        return await message.sendReply(
          `*_🌙 You are currently AFK_*\n\n` +
            ...
            `_Ecris n'importe quoi pour annuler l'AFK._`
        );
      } else {
        ...
        return await message.sendReply(
          `*_🌙 AFK reason updated_*\n\n` +
            `📝 _New reason:_ \`${input}\`\n\n` +
            `_T'inquiete boss je me charge de répondre à tes messages 😎._`
        );
      }
    } else {
      const reason = input || "je suis trop loin de mon clavier pour répondre";
      ...
      return await message.sendReply(
        `*_🌙 You're now AFK_*\n\n` +
          ...
          `_je vais répondre à tes messages en ton absence._\n` +
          `_Ecris ce que tu veux pour être de retour en ligne 👌._`
      );
    }
  }
);
    ...
    try {
      ...
      if (isAFK(senderJid)) {
        ...
        if (afkData) {
          ...
          const welcomeBack =
            `*_🌅 Bon retour !_*\n\n` +
            `⏰ _Tu étais absent(e) pendant:_ \`${timeAFK}\`\n` +
            `💬 _Messages reçu:_ \`${afkData.messageCount}\`\n` +
            `📝 _Ta raison:_ ${afkData.reason}`;
      ...
      if (isDM) {
        ...
        if (botOwnerJid && isAFK(botOwnerJid)) {
          ...
          const afkReply =
            `*_🌙 My owner is currently AFK_*\n\n` +
            `📝 _Reason:_ \`${afkData.reason}\`\n` +
            `⏰ _AFK for:_ \`${timeAFK}\`\n` +
            `👁️ _Last seen:_ \`${lastSeen}\`\n` +
            `💬 _Messages received:_ \`${afkData.messageCount + 1}\`\n\n` +
            `_Ton message à été noté nous reviendrons vers vous ultérieurement._`;
